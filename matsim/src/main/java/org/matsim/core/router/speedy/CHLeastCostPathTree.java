 ...
 *   <li><b>Phase 2 – Downward push sweep</b>: Process all nodes in <em>decreasing</em>
 *       rank order.  For each visited node u, push its cost along outgoing
 *       downward edges (u→w via {@code dnOutEdges}) to lower-ranked successors w.
 *       If {@code cost[u] + weight(u→w) < cost[w]}, update w.
 *       After the sweep, every node has its optimal shortest path cost.</li>
 * </ol>
 *
 ...
 *   <li><b>Phase 2 – Backward push sweep</b>: Process all nodes in decreasing rank
 *       order.  For each visited node u, push its cost along incoming upward
 *       edges (w→u via {@code upInEdges}) to lower-ranked predecessors w.
