 ...
 * {@link CHGraph}.
 *
 * <p>Per-node search state uses separate contiguous arrays for forward and
 * backward directions ({@code fwdCost[]}, {@code fwdComingFrom[]},
 * {@code fwdUsedEdge[]}, {@code fwdIterIds[]} and the corresponding
 * {@code bwd*} arrays).  Edge weights are read from colocated
 * {@code upWeights[slot]}/{@code dnWeights[slot]} arrays, eliminating
 * the global-index indirection on the hot path.
 *
 * @author Steffen Axer
 */
    ...
    private Path calcLeastCostPathImpl(int startIdx, int endIdx,
        ...
        while (!fwdPQ.isEmpty() || !bwdPQ.isEmpty()) {
            ...
            if (expandForward) {
                ...
                // via an upward edge to a higher-ranked node w already settled
