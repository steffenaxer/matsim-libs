 ...
 * @author mrieser
 */
public abstract class AbstractMATSimControler implements ControlerI, MatsimServices, AllowsConfiguration{
	...
	private static final Logger log = LogManager.getLogger(AbstractMATSimControler.class);
	 ...
	 */
	public AbstractMATSimControler(final String[] args) {
		this(args.length > 0 ? args[0] : null, null, null);
	}

	public AbstractMATSimControler(final String configFileName) {
		this(configFileName, null, null);
	}

	public AbstractMATSimControler(final Config config) {
		this(null, config, null);
	}

	public AbstractMATSimControler(final Scenario scenario) {
		this(null, null, scenario);
	}

	private AbstractMATSimControler(final String configFileName, final Config config, Scenario scenario) {
		if (scenario != null) {
	...
	// Load adjustable default modules
	...
	private void createInjector() {
		if (!injectorCreated) {
			this.loadDefaultModules(modules);
		...
		if (this.injector != null) {
			...
			return new EventsManager() {
				...
				public void processEvent(Event event) {
					AbstractMATSimControler.this.injector.getInstance(EventsManager.class).processEvent(event);
				}
	...
	@Override
	public final AbstractMATSimControler addOverridingModule( AbstractModule abstractModule ) {
		if (this.injectorCreated) {
	...
	@Override
	public final AbstractMATSimControler addOverridingQSimModule( AbstractQSimModule qsimModule ) {
		if (this.injectorCreated) {
			throw new RuntimeException("Too late for configuring the Controler. This can only be done before calling run.");
		}
	...
	@Override
	public final AbstractMATSimControler addQSimModule(AbstractQSimModule qsimModule) {
		this.addOverridingModule(new AbstractModule() {
			@Override
			public void install() {
     ...
     */
	@Override
	public final AbstractMATSimControler configureQSimComponents(QSimComponentsConfigurator configurator) {
		this.addOverridingModule(new AbstractModule() {
			@Override
			public void install() {
