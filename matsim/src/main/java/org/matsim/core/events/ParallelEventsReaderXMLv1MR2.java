 ...
 */
public class ParallelEventsReaderXMLv1MR2 extends MatsimXmlEventsParser {
	...
	public ParallelEventsReaderXMLv1MR2(EventsManager eventsManager) {
	...
	private static class EventData {
		...
		private EventData(CompletableFuture<Event> futureEvent, String type, String time) {
		...
		public EventData(CompletableFuture<Event> futureEvent, String type, String time, Attributes xmlAttributes, String... neededAttributes) {
			this(futureEvent, type, time);
			for (int i = 0; i < neededAttributes.length; i++) {
				String attributeName = neededAttributes[i];
				this.attributes.put(attributeName, xmlAttributes.getValue(attributeName));
			}
		}
