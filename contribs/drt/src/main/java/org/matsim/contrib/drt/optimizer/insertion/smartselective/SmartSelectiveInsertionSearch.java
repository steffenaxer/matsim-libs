...
/**
 * @author Steffen Axer, Michal Maciejewski
 */
final class SmartSelectiveInsertionSearch implements DrtInsertionSearch, MobsimBeforeCleanupListener {

	private final SmartSelectiveInsertionProvider insertionProvider;
	...
	private final LazyTravelTimeMatrix travelTimeCorrectionMatrix;

	public SmartSelectiveInsertionSearch(SmartSelectiveInsertionProvider insertionProvider,
										 SingleInsertionDetourPathCalculator detourPathCalculator, InsertionCostCalculator insertionCostCalculator,
										 DrtConfigGroup drtCfg, MatsimServices matsimServices, IncrementalStopDurationEstimator incrementalStopDurationEstimator, TravelTimeMatrix travelTimeMatrix, LazyTravelTimeMatrix travelTimeCorrectionMatrix) {
		...
		this.travelTimeCorrectionMatrix = travelTimeCorrectionMatrix;
	}
	...
	public Optional<InsertionWithDetourData> findBestInsertion(DrtRequest drtRequest,
		...
		updateLazyTravelTimeMatrix(drtRequest, travelTimeMatrix,  travelTimeCorrectionMatrix,  insertionWithDetourData);
	...
	private void updateLazyTravelTimeMatrix(DrtRequest request, TravelTimeMatrix travelTimeMatrix, LazyTravelTimeMatrix lazyTravelTimeMatrix, InsertionWithDetourData insertionWithDetourData) {
		updateLazyTravelTimeMatrix(request, travelTimeMatrix, travelTimeCorrectionMatrix, insertionWithDetourData.detourData.detourToPickup);
		updateLazyTravelTimeMatrix(request, travelTimeMatrix, travelTimeCorrectionMatrix, insertionWithDetourData.detourData.detourFromPickup);
		updateLazyTravelTimeMatrix(request, travelTimeMatrix, travelTimeCorrectionMatrix, insertionWithDetourData.detourData.detourToDropoff);
		updateLazyTravelTimeMatrix(request, travelTimeMatrix, travelTimeCorrectionMatrix, insertionWithDetourData.detourData.detourFromDropoff);
	}

	private static void updateLazyTravelTimeMatrix(DrtRequest request, TravelTimeMatrix travelTimeMatrix, LazyTravelTimeMatrix lazyTravelTimeMatrix, OneToManyPathSearch.PathData pathData) {
		if(pathData!=null && pathData.getPath().links!=null)
		{
			...
			lazyTravelTimeMatrix.setTravelTime(path.getFromNode(), path.getToNode(), ttRoutedEstimate, request.getEarliestStartTime());
		}
	}
